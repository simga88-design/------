...
/* eslint-disable @next/next/no-img-element -- User-uploaded Firebase profile images can be arbitrary external URLs. */
...
export default function ProfilePage() {
  ...
  const startEditing = () => {
    if (!profile) return;
    if ((profile.tickets || 0) <= 0) {
      alert('보유한 닉네임 교환권이 없어요. 아이디어 작성과 공감으로 레벨을 올려 10레벨 단위 보상을 받아보세요.');
      return;
    }
  ...
  const handleSaveName = async () => {
    ...
    if (nextName.length > 0) await updateNickname(nextName);
    setIsEditing(false);
  };
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (readerEvent) => {
      if (typeof readerEvent.target?.result === 'string') {
        updateProfileImage(readerEvent.target.result);
      }
    };
  ...
  if (!user || !profile) {
    return (
      <main className="pt-32 pb-32 px-4 sm:px-6 max-w-lg mx-auto relative min-h-screen text-center flex flex-col items-center">
        ...
        <h1 className="text-3xl font-black mb-4 font-headline text-slate-900 dark:text-white">로그인이 필요합니다</h1>
        <p className="text-slate-600 dark:text-slate-400 mb-10 font-bold leading-relaxed">
          아이디어를 쓰고, 공감하고, 작업방에 참여하려면 먼저 Google 계정으로 시작해주세요.
        </p>
        <button
          type="button"
          onClick={login}
          className="bg-white dark:bg-slate-800 text-slate-800 dark:text-white border-2 border-slate-200 dark:border-slate-700 px-8 py-4 rounded-full font-black text-lg shadow-md hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3"
        >
          <span className="material-symbols-outlined text-pink-500">login</span>
          Google 계정으로 시작하기
        </button>
      </main>
    );
  ...
  return (
    <main className="pt-24 pb-32 px-4 sm:px-6 max-w-lg mx-auto relative min-h-screen">
      ...
      <div className="bg-white/80 dark:bg-[#1a1625]/90 backdrop-blur-xl rounded-[2rem] p-6 shadow-2xl border border-pink-200/50 dark:border-pink-800/30 relative overflow-hidden group hover:shadow-pink-300/20 transition-all duration-500 max-w-sm mx-auto flex flex-col items-center">
        <div className="absolute inset-0 bg-gradient-to-tr from-pink-300/20 via-transparent to-indigo-300/20 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
        <div className="w-14 h-2.5 bg-slate-200/50 dark:bg-black/50 rounded-full mb-6 shadow-inner mx-auto mt-2" />
          ...
          <div className="flex flex-col items-center gap-2 mb-4 z-10 w-full px-4">
            <div className="flex w-full gap-2">
              <input
                ...
                autoFocus
              />
              <button type="button" onClick={handleSaveName} className="bg-indigo-500 hover:bg-indigo-600 text-white rounded-xl px-4 py-2 font-bold shadow-md active:scale-95 transition-all shrink-0">
                적용
              </button>
            </div>
            <p className="text-[11px] font-bold text-pink-600 dark:text-pink-400 bg-white/60 dark:bg-black/40 px-3 py-1 rounded-full shadow-sm mt-1">
              닉네임 교환권 1장이 사용됩니다.
            </p>
          </div>
        ) : (
        ...
        <div className="w-full bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-200/50 dark:border-slate-800 mb-6">
          <p className="text-slate-500 dark:text-slate-400 font-bold text-[11px] mb-1">누적 포인트</p>
          ...
          <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-3.5 relative overflow-hidden shadow-inner border border-slate-300/50 dark:border-slate-700/50">
            <div className="bg-gradient-to-r from-pink-400 to-indigo-500 h-full rounded-full transition-all duration-1000 ease-out relative" style={{ width: `${progressPercent}%` }}>
              <div className="absolute inset-0 opacity-40 mix-blend-overlay" style={{ backgroundImage: 'radial-gradient(circle, rgba(255,255,255,0.8) 1px, transparent 1px)', backgroundSize: '4px 4px' }} />
            </div>
          </div>
      ...
      <div className="mt-8 space-y-4 max-w-sm mx-auto">
        <h3 className="font-headline font-bold text-slate-700 dark:text-slate-300 px-2">설정</h3>
        <div className="bg-white/60 dark:bg-black/40 backdrop-blur-md rounded-2xl p-4 flex items-center justify-between shadow-sm border border-slate-200/50 dark:border-slate-800/50 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="font-bold text-slate-800 dark:text-slate-200 text-sm">다크 모드</h4>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">눈에 편한 화면으로 전환합니다.</p>
            </div>
          </div>
          <button
            type="button"
